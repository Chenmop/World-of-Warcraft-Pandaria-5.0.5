            ...
            void OnProc(constAuraEffectPtr aurEff, ProcEventInfo& eventInfo)
            {
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    ...
                    if (psyfiendList.size() > 1)
                        SurgeCore::Containers::RandomResizeList(psyfiendList, 1);
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        if (_player->HasAura(PRIEST_ATONEMENT_AURA))
                        {
                            ...
                            if (groupList.size() > 1)
                            {
                                groupList.sort(SurgeCore::HealthPctOrderPred());
                                groupList.resize(1);
                            }
            ...
            void HandleOnHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        SurgeCore::Containers::RandomResizeList(targetList, (affectedUnits * 2));
