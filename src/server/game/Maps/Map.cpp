...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = SurgeCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
{
    CellCoord cellCoord = SurgeCore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = SurgeCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<SurgeCore::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<SurgeCore::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    SurgeCore::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<SurgeCore::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<SurgeCore::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
...
{
    cell.SetNoCreate();
    SurgeCore::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<SurgeCore::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    SurgeCore::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<SurgeCore::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<SurgeCore::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
bool Map::ActiveObjectsNearGrid(NGridType const& ngrid) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellCoord p = SurgeCore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridCoord p = SurgeCore::ComputeGridCoord(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridCoord p2 = SurgeCore::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
